...
generated quantities {
  vector[K] log_lik_site;
  vector[J] log_lik_person;
...

  {
    ...
    log_lik_site = rep_vector(0, K);
    log_lik_person = rep_vector(0, J);
    for (n in 1:N) {
      real ll = normal_lpdf(y[n] | mu[n], sigma_person[person[n]]);
      log_lik_site[site[n]] += ll;
      log_lik_person[person[n]] += ll;
    }
  }

  // person-level coefficients (includes site + person deviations)
  for (j in 1:J) {
    beta_person[j] = to_row_vector(beta) + site_eff[person_site[j]] + person_eff[j];
  }
}
